...
class PlantDetailViewController: UIViewController {
    ...
    @IBAction func saveButtonTapped(_ sender: UIBarButtonItem) {
        guard let image = plantImageView.image, let name = plantNameTextField.text, !name.isEmpty else { return }
        let data = image.jpegData(compressionQuality: 0.5)
      
        if let passedInPlant = plant {
            //UPDATE PLANT
             print(" this is the date string format: \(datePickerProperties.date.formatDate())")
            PlantController.update(plant: passedInPlant, withNewName: name, withNewSchedule: datePickerProperties.date, withNewPhoto: data)
        }  else  {
            //CREATE NEW PLANT
//            guard let passedInOwner = owner else { return }
            let owner = Owner(username: "testOwner1", phoneNumber: "owner1@email.com", password: "password")
            print(" this is the date string format: \(datePickerProperties.date.formatDate())")
            PlantController.createPlantWith(name: name, photo: data, schedule: datePickerProperties.date, owner: owner)
        }
